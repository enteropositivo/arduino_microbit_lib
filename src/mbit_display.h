/*
  mbit_display.h - LED Screen library for micro:bit
...
#ifndef mbit_display_h
#define mbit_display_h
...
const MBIT_LED LED_FROM_5X5[rowCount][colCount] PROGMEM =
{
  {{0, 0}, {3, 1}, {1, 0}, {4, 1}, {2, 0}},
  {{3, 2}, {4, 2}, {5, 2}, {6, 2}, {7, 2}},
  {{1, 1}, {8, 0}, {2, 1}, {8, 2}, {0, 1}},
...
const uint8_t MBIT_FONT[96][colCount] PROGMEM =
{
  /* 0x20 - 0x2F */
...
const uint8_t MBIT_ICONS[40][colCount] PROGMEM =
{
...
const uint8_t MBIT_ARROWS[8][colCount] PROGMEM =
{
...
class mbit_display {
  private:
    ...
    void setBuffer(uint8_t row, uint8_t col, uint8_t data);
    uint8_t getBuffer(uint8_t row, uint8_t col);

    void update_showString();
    
    uint32_t buffer_data;  // 111111111 111111111 111111111  3x9 rows and cols  
    uint32_t screen_data;  // data actually showing in screen
  ...
  public:
    ...
    void showArrow(const ArrowNames direction);
